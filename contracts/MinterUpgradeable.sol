...
contract MinterUpgradeable is IMinter, OwnableUpgradeable {
    ...
    uint public weekly; // represents a starting weekly emission of 2.6M ZARD (ZARD has 18 decimals)
    ...
    function initialize(    
        ...
        _zard = IZard(IVotingEscrow(__ve).token());
        ...
        weekly = 30240 * 1e18; // represents a starting weekly emission of 30,240 $ZARD (ZARD has 18 decimals)
        isFirstMint = true;
    ...
    function _initialize(
        ...
        if(max > 0){
            _zard.mint(address(this), max);
            _zard.approve(address(_ve), type(uint).max);
    ...
    function circulating_supply() public view returns (uint) {
        return _zard.totalSupply() - _zard.balanceOf(address(_ve));
    }
    ...
    function calculate_rebate(uint _weeklyMint) public view returns (uint) {
        uint _veTotal = _zard.balanceOf(address(_ve));
        uint _zardTotal = _zard.totalSupply();
        
        uint lockedShare = (_veTotal) * PRECISION  / _zardTotal;
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) { // only trigger if new week
            ...
            uint _balanceOf = _zard.balanceOf(address(this));
            if (_balanceOf < _required) {
                _zard.mint(address(this), _required - _balanceOf);
            }

            require(_zard.transfer(team, _teamEmissions));
            
            require(_zard.transfer(address(_rewards_distributor), _rebase));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _zard.approve(address(_voter), _gauge);
