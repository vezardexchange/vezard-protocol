...
contract Pair is IPair {
    ...
    function _update0(uint amount) internal {
        ...
        uint256 _refFee = IPairFactory(factory).referralFee();
        uint256 _refFeeAmount = amount * _refFee / 10000;
        // get nft fee
        address _nftHandler = IPairFactory(factory).nftFeeHandler();
        uint256 _nftFee = IPairFactory(factory).nftFee();
        uint256 _nftFeeAmount = amount * _nftFee / 10000;
        if (_refFeeAmount > 0) {
            _safeTransfer(token0, _dibs, _refFeeAmount); // transfer the fees out to PairFees
            amount -= _refFeeAmount;
        }
        if (_nftFeeAmount > 0) {
            _safeTransfer(token0, _nftHandler, _nftFeeAmount); // transfer the fees out to PairFees
            amount -= _nftFeeAmount;
        }
        ...
        emit Fees(msg.sender, amount + _refFeeAmount + _nftFeeAmount, 0);
    }
    ...
    function _update1(uint amount) internal {
        ...
        uint256 _nftFeeAmount = amount * _nftFee / 10000;
        if (_refFeeAmount > 0) {
            _safeTransfer(token1, _dibs, _refFeeAmount); // transfer the fees out to PairFees
            amount -= _refFeeAmount;
        }
        if (_nftFeeAmount > 0) {
            _safeTransfer(token1, _nftHandler, _nftFeeAmount); // transfer the fees out to PairFees
            amount -= _nftFeeAmount;
        }
        ...
        emit Fees(msg.sender, 0, amount + _refFeeAmount + _nftFeeAmount);
    }
