...
contract RewardsDistributor is IRewardsDistributor {
    ...
    uint256 public instant_rate = 20;
    ...
    function _claim(uint _tokenId, address ve, uint _last_token_time) internal returns (uint) {
        ...
        emit Claimed(_tokenId, to_distribute, to_distribute * instant_rate / 100, user_epoch, max_user_epoch);
    ...
    function claim(uint _tokenId) external returns (uint) {
        ...
        if (amount != 0) {
            ...
            if(_locked.end < block.timestamp){
                ...
                uint instant_amount = amount * instant_rate / 100;
                if (instant_amount > 0) {
                    address _nftOwner = IVotingEscrow(voting_escrow).ownerOf(_tokenId);
                    IERC20(token).transfer(_nftOwner, instant_amount);
                }
                IVotingEscrow(voting_escrow).deposit_for(_tokenId, amount - instant_amount);
            }
    ...
    function setInstantRate(uint256 _rate) external {
        require(msg.sender == owner);
        require(_rate <= 100, "Exceeds max rate.");
        instant_rate = _rate;
    }
}
